...
class LinkedList : public List<E>{
    ...
    public:
        ...
        void radixSort(int radix){
            int digits = getMaxDigits();
            LinkedList<int> *buckets = new LinkedList<int> [radix];
            for(int i = 0; i < digits; i++){
                for(goToStart(); !atEnd(); next()){
                    int element = getElement();
                    int pos = element/powR(radix,i)%radix;
                    buckets[pos].append(element);
                }
                clear();
                for(int j = 0; j < radix; j++){
                    while(buckets[j].getSize() > 0){
                        append(buckets[j].remove());
                    }
                }
            }delete [] buckets;
        }
